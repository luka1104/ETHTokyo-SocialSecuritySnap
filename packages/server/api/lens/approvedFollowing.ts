...
const getApprovedAddressList = async (
  ...
  if (!contractDetails) {
    ResponseData +=
      '【WARNING】 The safety of the function you are trying to execute cannot be confirmed because it has not verified.';
    return ResponseData;
  }

  const contractFunctionDetails = await getContractFunctionDetails(
    inputData as string,
    contractDetails.abi,
  );
  ...
  if (functionName === 'setApprovalForAll') {
    approvedAddressList = await checkIsApprovedForAll(
      contractAddress,
      ownedByAddresses,
      functionArgs[0],
      Number(chainId),
...
const getRpcUrl = (chainId: any) => {
  switch (chainId) {
    ...
    case 137: {
      return process.env.INFURA_RPC_API_ETH_POLYGON as string;
    }
    case 80001: {
      return process.env.INFURA_RPC_API_ETH_MUMBAI as string;
    }
    case 59140: {
 ...
 */
const getContractDetails = async (chainId: number, contractAddress: string) => {
 ...
 */
const getContractFunctionDetails = (inputData: string, contractABI: string) => {
  const contractInterface = new ethers.Interface(contractABI);
  const decodedFunctionData = contractInterface.parseTransaction({
    data: inputData,
  });
  const functionName = (decodedFunctionData as ethers.TransactionDescription)
    .name;
  const functionArgs = (decodedFunctionData as ethers.TransactionDescription)
    .args;

  // Get the ABI of the executed function
  const arrContractABI = JSON.parse(contractABI) as ABI;
  let functionAbi = arrContractABI.find(
